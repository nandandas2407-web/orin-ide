...
function renderMD(text){
  ...
  const flushCode=(partial=false)=>{
    ...
    const previewBtn = (!partial && isHtml)
      ? `<button class="cb-copy cb-preview-html" onclick="ChatHTMLPreview.show(${JSON.stringify(content)})">Preview</button>`
      : '';
    const hdr=`<div class="cb-header">
      <span class="cb-lang">${esc(cLang)}</span>
      ${cPath?`<span class="cb-path">${esc(cPath)}</span>`:''}
      ${streamTag}
      ${!partial?`<button class="cb-copy" onclick="copyText(${JSON.stringify(content)})">Copy</button>`:''}
      ${previewBtn}
    </div>`;
  ...
  for(const line of lines){
    ...
    if(inCode){
      if(line==='```'||line.startsWith('```')){inCode=false;flushCode(false);}
