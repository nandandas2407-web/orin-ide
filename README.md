...
<details open>
<summary><strong>v1.0.9 — Agentic Mode Crash Fix</strong></summary>

**Agentic Mode**
- Fixed a bug where every agent turn threw `ReferenceError: systemContent is not defined`, which made Agentic Mode fail immediately on every send (the multi-agent pipeline never actually produced output)
- The agent's system prompt is now correctly preserved when the conversation is trimmed to fit the model's context budget, instead of being rebuilt from a variable that didn't exist in that scope
- Agentic Mode (Architect → Coder → Reviewer → Integrator) is now fully functional end-to-end, including auto-continue on cut-off responses and running `@@cmd` shell commands from the results
