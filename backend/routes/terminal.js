...
const BANNED_PATTERNS = [
  /\bmkfs\b/,                                          // format disks
  /\bdd\b.*\bif=.*\bof=\/dev\//,                      // write to raw device
  /:\(\)\s*\{.*:\s*\|.*:\s*&\s*\}.*:/,                // fork bomb
  /\brm\b.*--no-preserve-root/,                        // nuke filesystem root
];
// Everything else — including git, npm, pip, python, node, curl, etc — is allowed.
...
router.post('/exec', localOnly, (req, res) => {
  ...
  child.on('close', code => respondOnce({ stdout: stdoutBuf, stderr: stderrBuf, exitCode: code ?? 0 }));
  child.on('error', err => respondOnce({ stdout: stdoutBuf, stderr: stderrBuf + '\n' + err.message, exitCode: 1 }));
});
